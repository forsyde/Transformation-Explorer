...
brightness :: Image Double -- ^ input image
           -> [(Double, Double)] -- ^ tuple with 2 numbers: the minimum and
...
correctionFunc :: Int -> Int -> [Control] -> [(Double, Double)] -> [Double] -> [Double]
...
correctionSDF :: Int -- ^ dimension X for input images
              ...
              -> Signal (Double, Double) -- ^ stream of maxima and minima
              -> Signal Double -- ^ stream of input image pixels
              -> Signal Double -- ^ stream of output image pixels
correctionSDF dimX dimY = actor31SDF (1, 1, dimX * dimY) (dimX * dimY) (correctionFunc dimX dimY)
...
brightnessSDF :: Int -- ^ dimension X for input images
              ...
              -> Signal (Double, Double) -- ^ stream of maxima and minima
brightnessSDF dimX dimY = actor11SDF (dimX * dimY) 1 (brightness . toImage dimX dimY)
...
controlSDF :: Signal (Double, Double)
           -> Signal Control
controlSDF = mooreSDF ((3, 1), 3, maxMinToLevel) (3, 1, (averageOfLevels 3)) [255, 255, 255]
  where
    maxMinToLevel :: [Double] -> [(Double, Double)] -> [Double] -- revolving buffer with lists
