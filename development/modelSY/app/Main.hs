...
main = do
  ...
  let ppmFiles   = sort $ filter (isSuffixOf ".ppm") files
      inputPaths = checkEmpty $ map (\f->head path ++ "/" ++ f) ppmFiles :: [FilePath]
      checkEmpty [] = error  "the directory does not contain PPM files"
      checkEmpty a  = a
    in
      testBench inputPaths
  -- (x, y, imgs) <- readAllPPM inputPaths
  -- let positions = splitEvery 2 $ fromSignal $ imageProcessing $ signal imgs
...
parse argv
  = case getOpt Permute flags argv of
      (args,path,[]) -> do
        let options =
              let ops = concatMap (\x->filter (==x) args) [Coordinates]
